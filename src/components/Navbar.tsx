...
export default function Navbar() {
  const WHATSAPP_NUMBER = "8801984839526";

  const whatsappUrl = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(
    "Hi, I want to add my own photo frame."
  )}`;
  return (
    <>
      <nav className=" sticky top-0 w-full z-50 backdrop-blur-xl border-b  border-slate-200">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex gap-8 text-sm font-medium">
            {[
              { name: "Home", path: "/" },
              { name: "All Frame", path: "/all-frame" },
              { name: "Contact", path: `${whatsappUrl}` },
            ].map((link) => (
              <NavLink
                key={link.path}
                to={link.path}
          ...
          <Link
            ...
            className="group relative flex items-center gap-3 px-6 py-3 rounded-full 
             ...
             hover:shadow-[0_8px_25px_rgba(16,185,129,0.5)]
          ...
          >
            ...
            <span className="relative flex items-center gap-2">
              Add frame?
              <CirclePlus className="w-5 h-5 transition-transform duration-300 group-hover:rotate-90 text-emerald-100" />
            </span>

            {/* Notification-style dot (Optional Catchy element) */}
      ...
      <nav className="hidden p-4 md:px-12 flex justify-between items-center bg-white/70 backdrop-blur-xl sticky top-0 z-50 border-b border-gray-100">
        <Link to={"/"} className="cursor-pointer">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-green-600 rounded-lg flex items-center justify-center text-white font-black text-xl italic shadow-lg shadow-green-200">
              A
